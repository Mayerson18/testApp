...
export default {
  ...
  methods: {
    ...
    touchKey(key) {
      ...
      if (key === 'back') {
        var str = this.$store.state.discount.value.toString();
        var aux = str.substring(0, str.length - 1);
        total = parseFloat(parseInt(aux));
      } else {
        total = parseFloat(parseInt(this.$store.state.discount.value) + '' + key);
      }

      if (total) {
        if (total <= 100) {
          this.$store.state.discount.value = total;
        } else {
          this.setAlert('Monto invalido', false);
        }
      } else {
        this.$store.state.discount.value = 0;
      }
    },
